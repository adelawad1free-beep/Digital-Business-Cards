...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      const [settings, templates] = await Promise.all(promises);
  ...
  if (isCardDeleted) {
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center p-6 text-center ${isDarkMode ? 'bg-[#050507] text-white' : 'bg-slate-50 text-gray-900'}`}>
         <div className="w-32 h-32 bg-red-50 dark:bg-red-900/20 text-red-500 rounded-[3rem] flex items-center justify-center mb-10 shadow-2xl animate-bounce-in">
            <Trash2 size={56} />
         </div>
         <h1 className="text-4xl font-black mb-4 uppercase tracking-tighter">
            {isRtl ? 'هذه البطاقة تم حذفها' : 'Card Deleted'}
         </h1>
         <p className="text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-10 font-bold leading-relaxed">
            {isRtl 
              ? 'الرابط الذي تحاول الوصول إليه غير متاح حالياً، ربما قام صاحب البطاقة بحذفها أو تغيير الرابط الخاص بها.' 
              : 'The link you are trying to access is no longer available. The owner might have deleted it or changed the URL.'}
         </p>
         <a href="/" className="px-12 py-5 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase shadow-xl hover:scale-105 active:scale-95 transition-all">
            {isRtl ? 'أنشئ بطاقتك الخاصة الآن' : 'Create Your Own Card'}
         </a>
      </div>
    );
  }
